...
const LatLng SOURCE_LOCATION = LatLng(19.2839868,-99.4881235);
...
// for my drawn routes on the map
  ...
  String googleAPIKey = 'AIzaSyBQ4dmEAXS_6p5c-bgxKkatgNFqrLrS2A8';
Completer<GoogleMapController> _controller = Completer();
  ...
  UserCredential user; 
    ...
    print(position);
    setState(() {
          mylocation1 = LatLng(position.latitude, position.longitude);
        });
  }
/*
    void result() async {
     ...
     FirebaseDatabase.instance
      ...
      final long =double.parse(lonlat[1]);
       mylocation1 = LatLng(lat,long);
       print("mi localizacion $mylocation1");
    }

    */
  @override
  Widget build(BuildContext context) {
    final NotificacionesModel equiData=ModalRoute.of(context).settings.arguments;
    if (equiData != null) {
      notiModel = equiData;
      print(notiModel.geo);
      final lonlat=notiModel.geo.split(",");
      final lat=double.parse(lonlat[0]);
      final long =double.parse(lonlat[1]);
       geo = LatLng(lat,long);
       print("a donde vamos $geo");
    }
    ...
    setPolylines() async {
        ...
        PolylineResult result = await polylinePoints?.getRouteBetweenCoordinates(
            googleAPIKey,
            PointLatLng(mylocation1.latitude,
            mylocation1.longitude),
            PointLatLng(
            geo.latitude,
            geo.longitude));
